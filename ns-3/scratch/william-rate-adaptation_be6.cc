...
// ADICIONA OS BITS DO PACOTE A CONTAGEM TOTAL
void
...
// MUDA A POSICAO DO NOH PARA A A PASSADA NO PARAM
void
...
// GUARDA INFOS DO NOH E MANDA ELE PARA PROX POSICAO
void
NodeStatistics::AdvancePosition(Ptr<Node> node, int stepsSize, int stepsTime)
{
    ...
    m_bytesTotal = 0;
    // REGISTRA A POSICAO E A TAXA
    m_output.Add(pos.x, mbs);
    // ADICIONA OS PASSOS NA POSICAO
    pos.x += stepsSize;
    // ATUALIZA POSICAO DO NOH
...
// RETORNAS AS INFORMACOES GUARDADAS
Gnuplot2dDataset
...
{
    uint32_t rtsThreshold = 65535;  // TAMANHO LIMITE EM BYTES PARA Q SEJA NECESSARIO ENVIAR UM RTS
    std::string staManager = "ns3::MinstrelHtWifiManager";  // ALGORTMO DE ADAPTACAO DAS STAs
    std::string apManager = "ns3::MinstrelHtWifiManager";   // ALGORTMO DE ADAPTACAO DO AP
    
    // std::string standard = "802.11n-5GHz";  // PADRAO WI-FI USADO
    std::string standard = "802.11be";  // PADRAO WI-FI USADO
    
    std::string outputFileName = "minstrelHT";  // NOME DO ARQUIVO DE SAIDA DA SIMULACAO
    uint32_t BeMaxAmpduSize = 65535;    // VALOR MAXIMO EM BYTES PARA MPDU HABILITADO
    bool shortGuardInterval = false;    // SHORT GUARD INTERVAL INATIVO OU NAO
    uint32_t chWidth = 20;  // BANDA EM MHz
    int ap1_x = 0;  // POSICAO DO AP EM X
    int ap1_y = 0;  // POSICAO DO AP EM Y
    int sta1_x = 5; // POSICAO DO STA EM X
    int sta1_y = 0; // POSICAO DO STA EM Y
    int steps = 100;    // NUMERO DE PASSOS
    int stepsSize = 1;  // PASSO DE DISTANCIA EM METROS
    int stepsTime = 1;  // PASSO DE TEMPO EM SEGUNDOS

    // PEGA OS VALORES DO CMD CASO TENHA SIDO PASSADO POR LA
    ...
    if (standard != "802.11a" && standard != "802.11b" && standard != "802.11g" &&
        standard == "802.11n-2.4GHz" && standard != "802.11n-5GHz" && standard != "802.11ac" && standard != "802.11be")
    {
    ...
    // DEFINE A ERDA DE PROPAGACAO PARA 6GHZ 
    Config::SetDefault("ns3::LogDistancePropagationLossModel::ReferenceLoss",
                                           DoubleValue(48));
    
    // DEFINE A BANDA E FREQUENCIA USADA
    
    wifiPhy.Set("ChannelSettings",
                StringValue("{0, " + std::to_string(chWidth) + ", " + frequencyBand + ", 0}"));
    
    // NOTA: CCA, ou Clear Channel Assessment (Avaliação de Canal Limpo)
    ...
    else if (standard == "802.11n-2.4GHz" || standard == "802.11n-5GHz" || standard == "802.11ac" || standard == "802.11be")
    {
    ...
    // ADICIONA OS DISPOSIVOS

    wifiDevices.Add(wifiStaDevices);
    wifiDevices.Add(wifiApDevices);

    // DEFITE O INTERVALO DE GUARDA A PARTIR DO PARAMETRO

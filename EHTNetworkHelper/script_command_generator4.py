...
for i, params in enumerate(params_matrix):
    ...
    print(f"creating for: {freq}_{freq2}_{freq3} | emlsrLinks:{emlsrLinks}")
    sh_name = helper.generate_sh_script(
        output_sim_path=r"/results_teste/Sim_"+str(i+40)+str(f"_{int(freq)}")+str(f"_{int(freq2)}")+str(f"_{int(freq3)}"),
        sh_name=f"ns3_sim_{str(i+40)}_{int(freq)}_{int(freq2)}_{int(freq3)}",
        folder=pasta)
...
for i, params in enumerate(params_matrix):
    ...
    sh_name = helper.generate_sh_script(
        output_sim_path=r"/results_teste/Sim_"+str(i+50)+str(f"_{int(freq)}")+str(f"_{int(freq2)}")+str(f"_{int(freq3)}"),
        sh_name=f"ns3_sim_{str(i+50)}_{int(freq)}_{int(freq2)}_{int(freq3)}",
        folder=pasta)
    sh_names.append(sh_name)
...
for i, params in enumerate(params_matrix):
    ...
    print(f"creating for: {freq}_{freq2}_{freq3} | emlsrLinks:{emlsrLinks}")
    sh_name = helper.generate_sh_script(
        output_sim_path=r"/results_teste/Sim_"+str(i+60)+str(f"_{int(freq)}")+str(f"_{int(freq2)}")+str(f"_{int(freq3)}"),
        sh_name=f"ns3_sim_{str(i+60)}_{int(freq)}_{int(freq2)}_{int(freq3)}",
        folder=pasta)
    sh_names.append(sh_name)
